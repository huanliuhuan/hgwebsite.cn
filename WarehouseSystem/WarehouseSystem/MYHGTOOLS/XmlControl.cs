...
namespace MYHGTOOLS
{
    ...
    public class XmlControl
    {
        ...
        /// <returns></returns>
        public static bool AddNode(string filePath, string nodeName, string[] NodeAttr)
        {
            bool flag = false;
            ...
            {
                string NodeName = NodeAttr[i].Split(',')[0];
                string NodeValue = NodeAttr[i].Split(',')[1];
        ...
        public static DataTable GetTable(string source, XmlType xmlType)
        {
            DataSet ds = new DataSet();
            if (xmlType == XmlType.File)
            {
        ...
        public static object GetTableCell(string source, XmlType xmlType, int rowIndex, string colName)
        {
            DataSet ds = new DataSet();
            if (xmlType == XmlType.File)
            {
            ...
            return ds.Tables[0].Rows[rowIndex][colName];
        }
        ...
        /// <returns>值，不存在时返回Null</returns>
        public static object GetTableCell(string source, XmlType xmlType, int rowIndex, int colIndex)
        {
            DataSet ds = new DataSet();
            if (xmlType == XmlType.File)
            {
            ...
            return ds.Tables[0].Rows[rowIndex][colIndex];
        }
        ...
        /// <param name="NodeName">节点名称</param>
        /// <returns></returns>
        public static string GetWebSet(string NodeName)
        {
            string WebConfigPath = DirFile.GetMapPath(ConfigHelper.GetConfigString("WebConfig"));
            try
            {
                return XmlControl.GetNodeValue(WebConfigPath, XmlControl.XmlType.File, NodeName).ToString();
            }
            catch
            {
                return "";
