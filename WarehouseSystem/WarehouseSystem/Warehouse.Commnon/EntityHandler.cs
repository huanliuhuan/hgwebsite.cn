...
namespace Warehouse.Commnon
{
    ...
    public class EntityHandler<T> where T : new()
    {
        ...
        public static List<T> FillModel(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (DataColumn item in dt.Columns)
                {
                    ...
                    {
                        if (propertyInfo.PropertyType.IsGenericType) //泛型
                        {
                            //泛型Nullable<>
                            Type genericTypeDefinition = propertyInfo.PropertyType.GetGenericTypeDefinition();
                            if (genericTypeDefinition == typeof(Nullable<>))
                            {
                                propertyInfo.SetValue(model, Convert.ChangeType(dr[item.ColumnName], Nullable.GetUnderlyingType(propertyInfo.PropertyType)), null);
                            }
                        }
                        else
                        {
                            //非泛型
                            propertyInfo.SetValue(model, Convert.ChangeType(dr[item.ColumnName], propertyInfo.PropertyType), null);
                        }
                    }
                }
                modelList.Add(model);
        ...
        public static List<sParam> ConvertToparam(T Model)
        {
            List<sParam> Listp = new List<sParam>();
        ...
        public static string ConvertToString(T Model)
        {
            try
            {
        ...
        public static T ConvertToEntity(string Str)
        {
            try
            {
